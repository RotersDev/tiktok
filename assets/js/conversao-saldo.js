// JavaScript para página de Conversão Saldo Duplicado
...
if (typeof window.conversaoSaldoSupabase === 'undefined') {
    window.conversaoSaldoSupabase = null;
}

const WITHDRAWAL_VALUE = 2834.72; // Valor do saque
const DUPLICATED_VALUE = WITHDRAWAL_VALUE * 2; // Valor duplicado
const CONVERSION_FEE = 27.90; // Taxa de conversão
...
function getUrlParams() {
    ...
    return {
        ...
        amount: parseFloat(params.get('amount')) || WITHDRAWAL_VALUE
    };
}
...
async function createPixPayment(amount, payerName, pixKey, pixKeyType, supabaseClient = null) {
    try {
        const client = supabaseClient || window.conversaoSaldoSupabase;
        if (!client) {
        ...
        const { data, error } = await client.functions.invoke('bright-api', {
            body: {
                amount: amount,
                description: 'Conversão em Saldo Duplicado',
                payerName: payerName,
                pixKey: pixKey,
...
function initSupabase() {
    if (window.supabase && !window.conversaoSaldoSupabase) {
        try {
            window.conversaoSaldoSupabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
                auth: {
    ...
    return window.conversaoSaldoSupabase;
}
...
window.addEventListener('load', () => {
    ...
    const withdrawalValueEl = document.getElementById('withdrawalValue');
    const duplicatedValueEl = document.getElementById('duplicatedValue');

    if (withdrawalValueEl) withdrawalValueEl.textContent = formatCurrency(WITHDRAWAL_VALUE);
    if (duplicatedValueEl) duplicatedValueEl.textContent = formatCurrency(DUPLICATED_VALUE);
    ...
    async function processPixPayment() {
        try {
            ...
            const paymentData = await createPixPayment(CONVERSION_FEE, params.name, params.pixKey, params.pixKeyType, supabaseClient);

            if (paymentData && paymentData.paymentId) {
                monitorPaymentConfirmation(paymentData.paymentId, (confirmation) => {
                    showPixConfirmed(() => {
                        const nextUrl = `sacar.html?name=${encodeURIComponent(params.name)}&pixKey=${encodeURIComponent(params.pixKey)}&pixKeyType=${encodeURIComponent(params.pixKeyType)}`;
                        window.location.href = nextUrl;
                    });
                }, supabaseClient);

                showPixModal(paymentData, CONVERSION_FEE);
            } else {
    ...
    // Botão duplicar saldo
    const duplicateBalanceBtn = document.getElementById('duplicateBalanceBtn');
    if (duplicateBalanceBtn) {
        duplicateBalanceBtn.addEventListener('click', processPixPayment);
    }
